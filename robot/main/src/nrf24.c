...
static void nrf24_csn_high(void) { gpio_set_level(NRF24_PIN_CSN, 1); }
static void nrf24_csn_low(void)  { gpio_set_level(NRF24_PIN_CSN, 0); }

static esp_err_t nrf24_write_raw(const uint8_t *tx, uint8_t *rx, size_t len)
{
    if (nrf24_spi == NULL) return ESP_ERR_INVALID_STATE;
...
{
    if (tx == NULL || len == 0) return ESP_ERR_INVALID_ARG;
...
esp_err_t nrf24_init_irq(void)
{
    ...
    /* Clear any IRQ that fired between basic_config and ISR registration */
...
static esp_err_t nrf24_command(uint8_t cmd)
{
...
static esp_err_t nrf24_flush_tx(void) { return nrf24_command(NRF_CMD_FLUSH_TX); }
static esp_err_t nrf24_flush_rx(void) { return nrf24_command(NRF_CMD_FLUSH_RX); }
...
static esp_err_t nrf24_set_rf_setup(uint8_t rf_setup)
{
    return nrf24_write_reg(NRF_REG_RF_SETUP, rf_setup);
}

static esp_err_t nrf24_set_retries(uint8_t ard, uint8_t arc)
{
    if (ard > 0x0F || arc > 0x0F) return ESP_ERR_INVALID_ARG;
    return nrf24_write_reg(NRF_REG_SETUP_RETR, (uint8_t)((ard << 4) | arc));
}

static esp_err_t nrf24_set_address_width_5bytes(void)
{
    return nrf24_write_reg(NRF_REG_SETUP_AW, 0x03);
}

static esp_err_t nrf24_set_tx_address(const uint8_t *addr, size_t len)
{
    if (addr == NULL || len != NRF24_ADDR_LEN) return ESP_ERR_INVALID_ARG;
    return nrf24_write_buf(NRF_REG_TX_ADDR, addr, len);
}

static esp_err_t nrf24_set_rx_address_p0(const uint8_t *addr, size_t len)
{
    if (addr == NULL || len != NRF24_ADDR_LEN) return ESP_ERR_INVALID_ARG;
    return nrf24_write_buf(NRF_REG_RX_ADDR_P0, addr, len);
}

static esp_err_t nrf24_set_payload_width_p0(uint8_t width)
{
    if (width > NRF24_MAX_PAYLOAD_LEN) return ESP_ERR_INVALID_ARG;
    return nrf24_write_reg(NRF_REG_RX_PW_P0, width);
}

static esp_err_t nrf24_read_rx_payload(uint8_t *data, size_t len)
{
    if (data == NULL || len == 0 || len > NRF24_MAX_PAYLOAD_LEN) return ESP_ERR_INVALID_ARG;
...
esp_err_t nrf24_basic_config(const uint8_t *addr, uint8_t channel, uint8_t payload_len)
{
    ...
    ESP_ERROR_CHECK(nrf24_write_reg(NRF_REG_CONFIG,
